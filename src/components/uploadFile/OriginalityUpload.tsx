...
interface OriginalityUploadProps {
...
export const OriginalityUpload: React.FC<OriginalityUploadProps> = ({
  registrationId,
  existingUrl,
  qualificationEnd,
  onUploaded,
  ...
  const handleUpload = async () => {
    ...
    if (isDeadlinePassed()) {
      ...
      setMessage({
        type: "error",
        text: `Batas waktu pengumpulan orisinalitas telah berakhir pada ${deadlineText}. Upload tidak dapat dilakukan.`,
      });
      onError?.(
        `Batas waktu pengumpulan orisinalitas telah berakhir pada ${deadlineText}`
      );
      return;
    }
    ...
    try {
      ...
      const saveResult = await competitionService.submitOriginality(
        registrationId,
        result.data.secure_url
      );
      if (!saveResult.success) {
        const err = saveResult.error || "Gagal menyimpan orisinalitas";
        setMessage({ type: "error", text: err });
        onError?.(err);
        return;
      }
      ...
      setMessage({
        type: "success",
        text: "Orisinalitas berhasil diunggah dan disimpan",
      });
      setFile(null);
...
export default OriginalityUpload;
