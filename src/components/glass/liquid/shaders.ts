...
void main() {
  ...
  if (lLen > 0.001) lightDir2D = lightDir2D / lLen;

  float navAngle = dot(normalize(p + vec2(0.001, 0.001)), lightDir2D) * 0.5 + 0.5;
  float navSpec = pow(navAngle, 5.0) * 0.04;

  vec2 maxPillHalf = max(pillHalf, vec2(1.0, 1.0));
  vec2 pRel = vec2((p.x - pillCenter.x) / maxPillHalf.x, (p.y - pillCenter.y) / maxPillHalf.y);
  float pillAngle = dot(normalize(pRel + vec2(0.001, 0.001)), lightDir2D) * 0.5 + 0.5;
  ...
  vec2 navNorm = vec2(p.x / (u_resolution.x * 0.5), p.y / (u_resolution.y * 0.5));
  ...
  float motionDot = dot(normalize(pFromPill + vec2(0.001, 0.001)), motionLight) * 0.5 + 0.5;
