...
export class LiquidGlassRenderer {
  ...
  private compile(type: number, source: string): WebGLShader {
    ...
    const cleanSource = source.replace(/\r/g, '').trim();
    gl.shaderSource(shader, cleanSource);
    gl.compileShader(shader);
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      const info = gl.getShaderInfoLog(shader);
      const typeStr = type === gl.VERTEX_SHADER ? 'VERTEX' : 'FRAGMENT';
      console.error(`WebGL Shader compilation failed for ${typeStr}:`, info);
      console.error("Shader Source:", cleanSource);
      gl.deleteShader(shader);
      throw new Error(`Shader compile (${typeStr}): ` + info);
    }
    return shader;
  }
}
